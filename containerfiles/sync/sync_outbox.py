...
Connection handling: the LOCAL side (claim, mark-sent, return-to-pending) now
reuses ONE connection for the whole run. The WEBHOST side keeps its own
connection because it is a different server and must be its own transaction.
"""
import os
import sys
import time
...
def main():
    ...
    # One LOCAL connection reused for claim + mark-sent + return-to-pending.
    ...
    try:
        ...
        # --- 2. transmit to WEBHOST (separate transaction on its own connection) ---
        ...
        try:
            ...
            _return_batch(lc, bid, MAX_ATTEMPTS)
            return 1
        finally:
            wc.close()

        # --- 3. mark sent on LOCAL (reuse lc) ---
        _mark_sent(lc, bid)
        return 0
    finally:
        lc.close()
...
def _return_batch(local_conn, bid, max_attempts):
