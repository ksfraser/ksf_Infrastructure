...
## ⛔ MANDATORY RULES — READ FIRST (violations have caused production incidents)

1. **ALL code changes MUST be made in the DEV tree** — the module dev clones under
   `/home/kevin/Documents/<module-repo>` (e.g. `/home/kevin/Documents/ksf_FA_Common`,
   `/home/kevin/Documents/FA_ProductAttributes`).
   The clones under `fa_modules/` are **deployment bind copies only** — `git pull` /
   `git reset --hard origin/<branch>` to deploy, but **NEVER create, edit, or commit code there.**
   They are the *live* deployed modules on UAT; editing them silently diverges dev from production.

2. **ALWAYS COMMIT.** No uncommitted work may accumulate in any tree.
   Commit after each logical change; never leave files modified/untracked.

3. **Never fix production directly in the deployed clone.** If UAT/prod has a defect,
   reproduce/fix it in the DEV repo, commit there, push, then pull origin into the bind clone.

4. Rule #1 applies even when you are "just testing a quick fix" or "hot-patching UAT".
   There is no exception. If this rule is ever violated, the fix must be ported back into
   the DEV repo and the bind clone reset to origin before proceeding.

5. When asked to "deploy", pull origin into the bind clones — do not hand-edit them.
